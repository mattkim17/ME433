...
void initSPI() {
    ...
    // Pin B14 has to be SCK1 -> SCL (Can't be reassigned)
    
    // set SDO1 - B13 -> SDA (reassign)
    RPB13Rbits.RPB13R = 0b0011; 
   
    // set B15 -> RES (reset), initialize to High
    TRISBbits.TRISB15 = 0; //this makes it output
    LATBbits.LATB15 = 1; //this sets output to high
