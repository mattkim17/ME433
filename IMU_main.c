...
void imu_read(unsigned reg, signed short * data, int len){
    I2C_read_multiple(reg,data,len);
...
// Read from WHO_AM_I register 
    unsigned char WHO = readPin(WHO_AM_I, read_add);
    unsigned char imu[5];
    ...
    unsigned char acc = setPin(write_add, IMU_CTRL1_XL, 0x82); //set 1.66 kHz, with 2g sensitivity, and 100 Hz
    unsigned char gyro = setPin(write_add, IMU_CTRL2_G, 0x86); //1.66 kHz, with 1000 dps sensitivity.
    unsigned char cont = setPin(write_add, IMU_CTRL3_C, 0x04); // set IF_INC
    unsigned char message[100];
    
    if(0){
        sprintf(message, "g:, %d %d %d %d  ", imu[1], imu[2], imu[3]);
        drawString(message,0,0);
        sprintf(message, "a: %d %d %d  ", imu[4], imu[5], imu[6]);
        drawString(message, 0 ,16);
    }else{
        drawString(message,-imu[5], 1);
        drawString(message, imu[4], 1);
